    ...
    try:
        if settings.otel_traces_sampler != "parentbased_traceidratio":
            logger.warning(
                ...
                extra={"service": default_service_name, "sampler": settings.otel_traces_sampler},
            )
        ...
        logger.warning("OpenTelemetry tracing setup failed", extra={"service": default_service_name, "error_type": type(exc).__name__})
...
def instrument_fastapi(app) -> None:
    try:
        ...
        logger.warning("FastAPI tracing instrumentation failed", extra={"service": "api", "error_type": type(exc).__name__})
