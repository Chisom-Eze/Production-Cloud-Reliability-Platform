...
The API task role intentionally has no application AWS permissions in the current runtime. The API writes jobs and outbox records in PostgreSQL; it does not publish directly to SQS.
...
The worker task role can send, receive, delete, change visibility for, and inspect attributes of messages on the main SQS queue. It can also put and get report objects under the configured artifact object prefix.
...
The API task role does not receive SQS, S3, RDS, KMS, or Secrets Manager permissions. The worker task role owns both outbox dispatch and queue consumption, with SQS access scoped only to the main queue ARN and S3 object access scoped only to the configured report object prefix.
