...
def test_sqs_publisher_sends_stable_json_schema(monkeypatch):
    ...
    monkeypatch.setattr(
        "application.shared.adapters.boto3.client", lambda *_, **__: fake_client
    )
    job_id = uuid4()

    SqsJobPublisher("https://sqs.example/queue", "us-east-1").publish(
        job_id, "correlation-1"
    )
...
def test_sqs_consumer_rejects_unexpected_schema(monkeypatch):
    fake_client = FakeSqsClient(
        messages=[{"Body": '{"version":2}', "ReceiptHandle": "receipt"}]
    )
    monkeypatch.setattr(
        "application.shared.adapters.boto3.client", lambda *_, **__: fake_client
    )
...
def test_s3_artifact_store_puts_csv_without_acl_or_kms(monkeypatch):
    ...
    object_key, object_type = S3ArtifactStore(
        "artifact-bucket", "us-east-1"
    ).put_csv_report(
        job_id,
